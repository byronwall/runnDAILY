...
<div class="clear"></div>
<div class="grid_12">
<div class="actions">{{if $user_can_edit}} <a href="#new_event_modal" class="facebox icon"><img src="/img/icon/calendar_plus.png" />New Event</a> <a href="#new_announcement_modal" class="facebox icon"><img src="/img/icon/balloon_plus.png" />New
Announement</a> <a href="/routes/create?gid={{$group_view->gid}}" class="icon"><img src="/img/icon/route_plus.png" />New Route</a> <a href="#" class="icon"><img src="/img/icon/users_pencil.png" />Edit Group</a> {{/if}} {{if !($user_can_edit)}} {{if
$user_is_member}} <a id="a_join" href="#leave_group_modal" class="facebox icon"><img src="/img/icon/users_minus.png" />Leave Group</a> {{elseif $group_view->private}} <a href="#" class="icon"><img src="/img/icon/users_arrow.png" />Request to Join Group</a>
{{else}} <a id="a_join" href="#join_group_modal" class="facebox icon"><img src="/img/icon/users_arrow.png" />Join Group</a> {{/if}} {{/if}}</div>
</div>
<div class="clear"></div>
<div class="grid_3"><img src="/img/group/{{$group_view->imgsrc}}" class="fillx" />
<div class="box">
<h2>Details</h2>
...
<form action="/group/action_new_announcement" method="post" id="group_anoun_form"><input type="hidden" name="gid" value="{{$group_view->gid}}"> <input type="hidden" name="action" value="new_announcement" />
...
<h2>New Event</h2>
<form action="/group/action_new_event" method="post" id="group_event_form"><input type="hidden" name="g_id" value="{{$group_view->gid}}"> <input type="hidden" name="action" value="new_event" />
...
<p><input type="submit" value="Yes"> <input type="button" value="No" onclick="$.facebox.close()" /></p>
</form>
</div>
