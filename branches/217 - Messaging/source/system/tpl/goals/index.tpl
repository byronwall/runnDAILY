...
<div class="grid_12">
	...
	{{foreach from=$goal_list.active item=goal}}
		<p><a href="/goals/view/{{$goal->id}}/{{$goal->name}}" class="icon"><img src="/img/icon/trophy.png" />{{if $goal->name}}{{$goal->name}}{{else}}Unamed{{/if}} ({{$goal->percent|round:"1"}}%)</a></p>
	...
	{{foreach from=$goal_list.past item=goal}}
		<p><a href="/goals/view/{{$goal->id}}/{{$goal->name}}" class="icon"><img src="/img/icon/trophy_silver.png">{{if $goal->name}}{{$goal->name}}{{else}}Unamed{{/if}} ({{$goal->percent|round:"1"}}%)</a></p>
		{{if $goal->desc}}<p>{{$goal->desc}}</p>{{/if}}
	{{/foreach}}
	{{/if}}
</div>
