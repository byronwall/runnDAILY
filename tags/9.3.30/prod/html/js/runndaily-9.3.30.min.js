 ...
 */
(function($){function toIntegersAtLease(n){return n<10?"0"+n:n}Date.prototype.toJSON=function(date){return this.getUTCFullYear()+"-"+toIntegersAtLease(this.getUTCMonth())+"-"+toIntegersAtLease(this.getUTCDate())};var escapeable=/["\\\x00-\x1f\x7f-\x9f]/g;var meta={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"};$.quoteString=function(string){if(escapeable.test(string)){return'"'+string.replace(escapeable,function(a){var c=meta[a];if(typeof c==="string"){return c}c=a.charCodeAt();return"\\u00"+Math.floor(c/16).toString(16)+(c%16).toString(16)})+'"'}return'"'+string+'"'};$.toJSON=function(o,compact){var type=typeof (o);if(type=="undefined"){return"undefined"}else{if(type=="number"||type=="boolean"){return o+""}else{if(o===null){return"null"}}}if(type=="string"){return $.quoteString(o)}if(type=="object"&&typeof o.toJSON=="function"){return o.toJSON(compact)}if(type!="function"&&typeof (o.length)=="number"){var ret=[];for(var i=0;i<o.length;i++){ret.push($.toJSON(o[i],compact))}if(compact){return"["+ret.join(",")+"]"}else{return"["+ret.join(", ")+"]"}}if(type=="function"){throw new TypeError("Unable to convert object of type 'function' to json.")}var ret=[];for(var k in o){var name;type=typeof (k);if(type=="number"){name='"'+k+'"'}else{if(type=="string"){name=$.quoteString(k)}else{continue}}var val=$.toJSON(o[k],compact);if(typeof (val)!="string"){continue}if(compact){ret.push(name+":"+val)}else{ret.push(name+": "+val)}}return"{"+ret.join(", ")+"}"};$.compactJSON=function(o){return $.toJSON(o,true)};$.evalJSON=function(src){return eval("("+src+")")};$.secureEvalJSON=function(src){var filtered=src;filtered=filtered.replace(/\\["\\\/bfnrtu]/g,"@");filtered=filtered.replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]");filtered=filtered.replace(/(?:^|:|,)(?:\s*\[)+/g,"");if(/^[\],:{}\s]*$/.test(filtered)){return eval("("+src+")")}else{throw new SyntaxError("Error parsing JSON, source is not valid.")}}})(jQuery);(function($){$.fn.sortTable=function(options){var settings=$.extend({},$.fn.sortTable.defaults,options);var $table=$(this);$("th",$table).each(function(column){var findSortKey;if($(this).is(".sort-alpha")){findSortKey=function($cell){return $cell.text().toUpperCase()}}else{if($(this).is(".sort-numeric")){findSortKey=function($cell){var key=parseFloat($cell.text().replace(/^[^\d.]*/,""));return isNaN(key)?0:key}}else{if($(this).is(".sort-date")){findSortKey=function($cell){return Date.parse($cell.text())}}}}if(findSortKey){$(this).click(function(){if(settings.sort_field==$(this).text()){settings.sort_desc=-settings.sort_desc}else{settings.sort_field=$(this).text();settings.sort_desc=1}var rows=$table.find("tbody > tr").get();$.each(rows,function(index,row){row.sortKey=findSortKey($(row).children("td").eq(column))});rows.sort(function(a,b){if(a.sortKey<b.sortKey){return -settings.sort_desc}if(a.sortKey>b.sortKey){return settings.sort_desc}return 0});$.each(rows,function(index,row){$table.children("tbody").append(row);row.sortKey=null});if(settings.should_restripe){$table.restripe()}})}})};$.fn.restripe=function(){$("tbody tr:odd",this).removeClass("even").addClass("odd");$("tbody tr:even",this).removeClass("odd").addClass("even");return this};$.fn.sortTable.defaults={sort_field:null,sort_desc:-1,should_restripe:true}})(jQuery);var Chart={Data:{Distance_Max:0,Pace_Max:0,Date_Max:0,Date_Min:0},ActivePoint:{},Type:"distance",CurrentRange:{xaxis:{},yaxis:{from:0}},OverviewRange:{xaxis:{},yaxis:{from:0}},LoadData:function(chart_data){Chart.Data=chart_data;Chart.CurrentRange.xaxis.to=Chart.Data.Date_Max;Chart.CurrentRange.xaxis.from=Chart.Data.Date_Max-(8.75*24*60*60*1000);Chart.CurrentRange.yaxis.to=Chart.Data.Distance_Max;Chart.OverviewRange.xaxis.to=Chart.Data.Date_Max;Chart.OverviewRange.xaxis.from=Chart.Data.Date_Min;Chart.OverviewRange.yaxis.to=Chart.Data.Distance_Max},UpdateOverviewSelection:function(){Chart.overview.setSelection(Chart.CurrentRange,true)},UpdatePrimary:function(){if(Chart.Type=="distance"){ChartData=Chart.Data.Distance_Data;ChartOptions=ChartSettings.Primary.Distance;ChartMax=Chart.Data.Distance_Max}else{ChartData=Chart.Data.Pace_Data;ChartOptions=ChartSettings.Primary.Pace;ChartMax=Chart.Data.Pace_Max}NewMax=1;$.each(ChartData,function(i){if(this[0]>Chart.CurrentRange.xaxis.from&&this[0]<Chart.CurrentRange.xaxis.to&&this[1]>Chart.NewMax){Chart.NewMax=Math.ceil(this[1])}});if(Chart.Type=="distance"){Chart.Distance_Max=NewMax}else{Chart.Pace_Max=NewMax}Chart.primary=$.plot($("#primary_chart"),[ChartData],$.extend(true,{},ChartOptions,{xaxis:{min:Chart.CurrentRange.xaxis.from,max:Chart.CurrentRange.xaxis.to},yaxis:{min:0,max:ChartMax}}));$.each(ChartData,function(i){if(Chart.ActivePoint[i]){Chart.primary.highlight(0,i)}})},UpdateOverview:function(){if(Chart.Type=="distance"){ChartData=Chart.Data.Distance_Data;ChartOptions=ChartSettings.Overview.Distance;ChartMax=Chart.Data.Distance_Max}else{ChartData=Chart.Data.Pace_Data;ChartOptions=ChartSettings.Overview.Pace;ChartMax=Chart.Data.Pace_Max}Chart.overview=$.plot($("#overview_chart"),[ChartData],$.extend(true,{},ChartOptions,{xaxis:{min:Chart.OverviewRange.xaxis.from,max:Chart.OverviewRange.xaxis.to},yaxis:{min:0,max:ChartMax}}));Chart.UpdateOverviewSelection()},ToggleItem:function(event,pos,item){if(item){if($("#item_"+item.dataIndex).hasClass("active_row")){Chart.primary.unhighlight(item.series,item.datapoint);$("#item_"+item.dataIndex).removeClass("active_row");Chart.ActivePoint[item.dataIndex]=0}else{Chart.primary.highlight(item.series,item.datapoint);$("#item_"+item.dataIndex).addClass("active_row");Chart.ActivePoint[item.dataIndex]=1}}}};var ChartSettings={AdjustPrimaryRange:function(xmin,xmax,ymax){if(Chart.Type=="distance"){ChartOptions=ChartSettings.Primary.Distance}else{ChartOptions=ChartSettings.Primary.Pace}ChartOptions.xaxis.min=xmin;ChartOptions.xaxis.max=xmax;ChartOptions.yaxis.max=ymax},AdjustOverviewRange:function(xmin,xmax,ymax){if(Chart.Type=="distance"){ChartOptions=ChartSettings.Overview.Distance}else{ChartOptions=ChartSettings.Overview.Pace}ChartOptions.xaxis.min=xmin;ChartOptions.xaxis.max=xmax;ChartOptions.yaxis.max=ymax},Primary:{Distance:{xaxis:{mode:"time",timeformat:"%b %d",minTickSize:[1,"day"],min:0,max:1},yaxis:{max:1,min:0},bars:{show:true,barWidth:(24*60*60*1000),align:"center"},grid:{clickable:true}},Pace:{xaxis:{mode:"time",timeformat:"%b %d",minTickSize:[1,"day"],min:0,max:1},yaxis:{max:1,min:0},points:{show:true,radius:5},lines:{show:true},grid:{clickable:true}}},Overview:{Distance:{xaxis:{mode:"time",timeformat:"%b %d",minTickSize:[0.5,"month"],min:0,max:1},yaxis:{ticks:null,min:0,max:1},bars:{show:true,align:"center"},grid:{backgroundColor:"#ffffff"},selection:{mode:"x"}},Pace:{xaxis:{mode:"time",timeformat:"%b %d",minTickSize:[0.5,"month"],min:0,max:1},yaxis:{ticks:null,min:0,max:1},lines:{show:true},grid:{backgroundColor:"#ffffff"},selection:{mode:"x"}}}};
/*
 ...
 *
 */
(function($){$.fn.heightEqual=function(DOM){$(this).height($(DOM).height())};$.fn.heightBrowser=function(options){var settings=$.extend({},{scale:0.95},options);$(this).height($(window).height()*settings.scale);if(settings.callback){settings.callback()}};$.debug=function(message){if(window.console){console.log(message)}}})(jQuery);var Units={convert:1.6,is_mile:true,unit_value:"mi",unit_class:".dist-unit",dist_class:".dist-num",combined_class:".dist-val",callback:null,changeUnits:function(){if(Units.is_mile){Units.unit_value="km";Units.convert=1.609344}else{Units.unit_value="mi";Units.convert=0.6214}Units.is_mile=!Units.is_mile;$(Units.unit_class).text(Units.unit_value);$(Units.dist_class).each(function(){var dist=parseFloat($(this).text())*Units.convert;$(this).text(dist.toFixed(2))});$(Units.combined_class).each(function(){var dist=parseFloat($(this).text().replace(/^[^\d.]*/,""))*Units.convert;$(this).text(dist.toFixed(2)+" "+Units.unit_value)});if(Units.callback){Units.callback()}},init:function(options){Units=$.extend({},Units,options);$(Units.dist_class).die("click").live("click",Units.changeUnits);$(Units.combined_class).die("click").live("click",Units.changeUnits);$(Units.unit_class).die("click").live("click",Units.changeUnits)},textWithUnits:function(options){opts=$.extend({},{from_miles:true,add_units:false},options);if(!Units.is_mile){opts.dist*=Units.convert}$(opts.target).text(opts.dist.toFixed(2)).addClass(Units.dist_class.replace(".",""))}};var sorter={sort:function(key){if(!sorter.settings.classes[key]){return false}sorter.settings.sort_key=key;var items=$(sorter.settings.item,sorter.settings.parent).get();items.sort(function(a,b){var a_val=$(a).find("."+key).eq(0).text().toUpperCase();var b_val=$(b).find("."+key).eq(0).text().toUpperCase();if(sorter.settings.classes[key]=="numeric"){a_val=parseFloat(a_val.replace(/^[^\d.]*/,""));b_val=parseFloat(b_val.replace(/^[^\d.]*/,""))}else{if(sorter.settings.classes[key]=="date"){a_val=Date.parse(a_val);b_val=Date.parse(b_val)}}if(a_val<b_val){return -sorter.settings.sort_desc}if(a_val>b_val){return sorter.settings.sort_desc}return 0});$.each(items,function(){$(sorter.settings.parent).append(this)})},reverse:function(){sorter.settings.sort_desc=-sorter.settings.sort_desc;sorter.sort(sorter.settings.sort_key)},init:function(settings){sorter.settings=$.extend({},sorter.settings,settings)},settings:{classes:{},parent:null,item:null,sort_desc:-1,sort_key:null}};