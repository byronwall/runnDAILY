...
<div class="grid_12">
	<h4>Conversations</h4>
	<?php if(count($this->_vars["convo_list"])): foreach($this->_vars["convo_list"] as $this->_vars['convo']): ?>
	<p><a href="/messages/view_convo/<?php echo $this->_vars["convo"]['msg_convo_id']; ?>_modal" class="facebox icon"><?php if($this->_vars["convo"]['msg_uid_to'] == $this->_vars["currentUser"]->uid && $this->_vars["convo"]['msg_new']): ?><img src="/img/icon/mail_new.png" /><?php else: ?><img src="/img/icon/mail.png" /><?php endif ?><?php echo $this->_vars["convo"]['msg_subject']; ?></a> 	with
	<?php if($this->_vars["currentUser"]->uid == $this->_vars["convo"]['msg_uid_to']): ?>
		<a href="/community/view_user/<?php echo $this->_vars["convo"]['msg_uid_from']; ?>" class="icon"><img src="/img/icon/user_friend.png" /><?php echo $this->_vars["convo"]['msg_username_from']; ?></a>
	<?php endif ?>
	<?php if($this->_vars["currentUser"]->uid == $this->_vars["convo"]['msg_uid_from']): ?>
		<a href="/community/view_user/<?php echo $this->_vars["convo"]['msg_uid_to']; ?>" class="icon"><img src="/img/icon/user_friend.png" /><?php echo $this->_vars["convo"]['msg_username_to']; ?></a>
	<?php endif ?></p>
	<p>last activity: <?php echo mod_date_format($this->_vars["convo"]['msg_date_created'], "l F j, Y g:i A"); ?></p>
	<?php endforeach; else: ?>
